...
import { Upload, CheckCircle, Loader2, CreditCard, Image as ImageIcon } from "lucide-react";
...
export function PaymentConfirmDialog({
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      toast.success("Đã gửi xác nhận thanh toán! Chúng tôi sẽ liên hệ bạn sớm.");
      onSuccess();
      onOpenChange(false);
    } catch (error: any) {
        ...
        <div className="grid md:grid-cols-2 gap-6 mt-4">
          ...
          <div className="space-y-4">
            ...
            <div className="bg-accent/20 p-4 rounded-lg space-y-3">
              <div>
                <Label className="text-muted-foreground text-sm">Chủ tài khoản</Label>
                <p className="font-semibold text-lg">{bankConfig.accountName}</p>
              </div>
              <div>
                <Label className="text-muted-foreground text-sm">Số tài khoản</Label>
                <p className="font-semibold text-lg font-mono">{bankConfig.accountNumber}</p>
              </div>
              <div>
                <Label className="text-muted-foreground text-sm">Ngân hàng</Label>
                <p className="font-semibold">{bankConfig.bankName}</p>
              </div>
              <div className="pt-2 border-t border-border">
                <Label className="text-muted-foreground text-sm">Nội dung chuyển khoản</Label>
                <p className="font-semibold text-primary">{transferContent}</p>
              </div>
              <div className="pt-2">
                <Label className="text-muted-foreground text-sm">Số tiền đặt cọc</Label>
            ...
            {bankConfig.qrUrl && (
              <div className="text-center">
                <Label className="text-muted-foreground text-sm">Quét mã QR để chuyển khoản</Label>
          ...
          <div className="space-y-4">
            <h3 className="font-semibold text-lg">Tải ảnh xác nhận chuyển khoản</h3>
